...
inline void vec_scalar_mult(vector &dest, vector &src, double f) {                // f*a
  dest[0] = src[0]*f;
  dest[1] = src[1]*f;
  dest[2] = src[2]*f; }
...
inline void lvec_to_vec(vector &dest, lvector &src) {
 ...
 inline void vec_direct(shape &dest, vector &a, vector &b) {
 	dest[0] = a[0]*b[0];
 	dest[1] = a[1]*b[1];
 	dest[2] = a[2]*b[2];
 	dest[3] = a[0]*b[1];
...
// shape operators
inline double shape_trace(shape &s) {
	return s[0]+s[1]+s[2];
}
...
inline void shape_add(shape &dest, shape &src) {                // h_a+h_b
 ...
 inline void shape_add_trace(shape &dest, double f) {                // h_a+h_b
  dest[0] += f; 
  dest[1] += f; 
  dest[2] += f; 
 }
...
inline void shape_mult(shape &dest, double f) {
	dest[0] *= f;
	dest[1] *= f;
	dest[2] *= f;
	dest[3] *= f;
...
inline void shape_mult(shape &dest, shape &src, double f) {
	dest[0] = src[0]*f;
	dest[1] = src[1]*f;
	dest[2] = src[2]*f;
	dest[3] = src[3]*f;
...
inline void vec_dot_shape(vector &dest, vector &a, shape &b) {
	dest[0] = a[0]*b[0] + a[1]*b[3] + a[2]*b[4];
	dest[1] = a[0]*b[3] + a[1]*b[1] + a[2]*b[5];
	dest[2] = a[0]*b[4] + a[1]*b[5] + a[2]*b[2];
}
